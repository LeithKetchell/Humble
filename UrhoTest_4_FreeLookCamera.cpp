...
/// Free-Look camera behaviour / more device input stuff
/// Load/Save scene content via XML file
...
public:
   ...
   //     GetSubsystem<Input>()->SetMouseVisible(true);
    ...
    /// VariantMap is an alias for "Map<StringHash, Variant>" where a "StringHash" is a unique number generated from a plaintext string.
    /// Using "hashes" as keys, rather than actual strings, is a performance optimization.
    /// Variant is an Urho value container that can hold almost any kind of value or object.
    /// These will likely become more familiar in time, but know these are all Urho classes.
    ...
    /// This is not the only way to detect and handle keyboard stuff,
    /// consider this event a kind of "catch-all" for handling the most important keypresses
...
private:
    ...
    void MoveCamera(float timeStep)
    {
        ...
        /// Mouse sensitivity (degrees per pixel: will need to be adjusted to suit display resolution)
