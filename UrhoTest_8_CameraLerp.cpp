...
private:
    ...
    */
    ...
    Node* CreateCamera(const Vector3& Position){
        gameScene_->CreateChild("Camera Node");
        cameraNode_->SetWorldPosition(Position);           /// Move camera to initial position
    ...
    void HandleKeyDown(StringHash eventType, VariantMap& eventData){
        ...
        /// When gui window is hidden, SPACE toggles the Camera Behaviour
        else if(key==KEY_SPACE && !window_->IsVisible())
        {
            switch(cameraBehaviour_){
                case FreeLook:
    ...
    void HandlePostRenderUpdate(StringHash eventType, VariantMap& eventData){
        ...
        /// Let's also debug-draw the local axes of our "player character"
